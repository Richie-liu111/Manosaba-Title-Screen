...
public class ExitDialog {
    ...
    public ExitDialog(VirtualScreen vs) {
        ...
        cancelButton.setClickSound(ManosabaSounds.SFX_SYSTEM_CANCEL.value());
    ...
    private void quit() {
        ...
        mc.getSoundManager().play(SimpleSoundInstance.forUI(ManosabaSounds.SFX_SYSTEM_SUBMIT.value(), 1.0f, 1.0f));
    }
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        ...
        RenderUtils.blit(g, TextureConst.BLACK, 0, 0, g.guiWidth(), g.guiHeight(), CURTAIN_ALPHA / 255f);

        // 2) 亮色条带 = Frame 区域（纹理行 370..1070 → y 370..1070），完全不透明。
        //    走 13 参 blitCrop：源区 = dialogbase (0,370)-(2560,1070)，缩放到 2560×700。
        RenderUtils.blitCrop(g, TextureConst.DIALOG_BASE,
                vs.toPracticalX(0), vs.toPracticalY(BAND_Y),
                vs.toPracticalWidth(2560), vs.toPracticalHeight(BAND_H),
                0, BAND_Y, 2560, (int) BAND_H, 2560, 1440, alpha);
        ...
        // 退出黑幕由 ManosabaTitleScreen 在对话框之后以入场同机制绘制（blit 路径）
    }

    /** 对话框打开时优先消费鼠标点击，屏蔽底层按钮。 */
    public boolean mouseClicked(MouseButtonEvent event, boolean b) {
        if (state == State.OPEN || state == State.FADE_IN) {
            return cancelButton.mouseClicked(event, b)
                    | endButton.mouseClicked(event, b);
        }
        return false;
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        if ((state == State.OPEN || state == State.FADE_IN) && event.key() == GLFW.GLFW_KEY_ESCAPE) {
            close();
            return true;
        }
        return false;
